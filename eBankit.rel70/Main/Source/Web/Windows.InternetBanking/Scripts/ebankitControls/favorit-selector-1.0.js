...
resizeend = function () {
	if (new Date() - rtime < delta) {
		...
		timeout = false;
		if ((isMobile && window.matchMedia("(min-width: 767px)").matches) || (!isMobile && window.matchMedia("(max-width: 766px)").matches)) {
			FavoritSelector.LoadSlider(true);
		}
	}
}
...
FavoritSelector.LoadSlider = function (reloadSelector) {
    ...
    if (reloadSelector) {
        ...
        slider = $('.bxsliderFavorits_FWBC').bxSlider(options);
    }
};
