<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CardControl.ascx.cs" Inherits="CardControl" %>
...
<div class="scrollContainerAccounts">
    <div id="scroolContent" class="col-xs-12 col-sm-12 col-md-10 col-lg-8 col-max controlsShowHide metro content-accounts" style="overflow: hidden; visibility: hidden;">
        <asp:UpdatePanel ID="flwContainerCards" runat="server" UpdateMode="Always">
            <Triggers>
                <asp:AsyncPostBackTrigger ControlID="btnSelectedCardChanged" />
            </Triggers>
            <ContentTemplate>
                <div>
                    <div class="slickSlider" style="position: relative;">
                        <asp:Repeater ID="rptCreditCards" runat="server" OnItemDataBound="rptCreditCards_ItemDataBound">
                            <ItemTemplate>
                                <div id="divCard" role="button" tabindex="0" onkeydown="eBankit.Presentation.AccountsControl.CardKeydownHandler(this, event);" onkeyup="eBankit.Presentation.AccountsControl.CardKeyupHandler(this, event);" runat="server">
                                    <div id="cardSlide" runat="server" class="contentBox box-blue">
...
<asp:UpdatePanel ID="UpdatePanel1" UpdateMode="Always" runat="server">
    <ContentTemplate>
        ...
        <it:TabSelectorControl
            ID="TabSelectorCardAccount"
            ControlName="CardAccountControl"
            Visible="false"
            runat="server"
            titleOperations="<%$FrontEndResources:TRX00211,titleOperations%>"
            titleFavorits="<%$FrontEndResources:TRX00211,titleFavorits%>" />
    </ContentTemplate>
</asp:UpdatePanel>
...
<script type="text/javascript">
    ...
    eBankit.Presentation.AccountsControl.CardKeydownHandler = function (elem, event) {
        if (event.keyCode === 32) {
    ...
    eBankit.Presentation.AccountsControl.CardKeyupHandler = function (elem, event) {
        if (event.keyCode === 32) {
            event.preventDefault();
            $(elem).click();
        }
